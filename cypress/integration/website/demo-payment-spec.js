...
describe('Flow Payment', function () {
    ...
    it('Search a product', function () {
        if (Cypress.env('platform') === "desktop") {
            cy.wait(1000);
            cy.percySnapshot('Product',{ scope: 'div[data-qa="search_input"]' });
    ...
    it('Enter the credit cart', function () {
        ...
        if (Cypress.env('platform') === "mobile") {
            cy.viewport(414,896);
            cy.frameLoaded(".processout-field-cc-iframe");
            cy.iframe().get('input[id="processout-field"]').should('be.visible').type("444444444444444");
        }
    });
